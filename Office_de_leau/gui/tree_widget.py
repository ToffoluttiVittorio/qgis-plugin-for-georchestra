...
##            i += 1
    ...
    def filter_by_extent(self, currentindex):
        ...
        if currentindex == 1:
            ...
            # convert canvas bbox to wgs84
            sourceCrs = QgsCoordinateReferenceSystem(QgsProject.instance().crs().authid())
            destCrs = QgsCoordinateReferenceSystem(4326)
            tr = QgsCoordinateTransform(sourceCrs, destCrs, QgsProject.instance())
            map_extent_wgs84 = tr.transformBoundingBox(map_extent)
            # turns bbox into a list [xmin, xmax, ymin, ymax]
            bbox_map = [map_extent_wgs84.xMinimum(), map_extent_wgs84.xMaximum(), map_extent_wgs84.yMinimum(), map_extent_wgs84.yMaximum()]
            # expands all folders
            self.expandAll()
